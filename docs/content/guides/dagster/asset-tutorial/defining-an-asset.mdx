...
If you'd rather materialize your asset as a script, you can do that without spinning up Dagit. Just add a few lines to `cereal.py`. This collects all the assets in the current module (in this case just the single asset we defined above) and then launches an in-process run to materialize them.
