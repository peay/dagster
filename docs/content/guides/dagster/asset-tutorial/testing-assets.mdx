...
We'll start by writing a test for the `nabisco_cereals` asset definition, which filters the larger list of cereals down to the those that were manufactured by Nabisco. To run the op (function) that derives an asset from its upstream dependencies, we can invoke it directly, as if it's a regular Python function:
